...
def handle_users():
    ...
    if request.method == 'GET':
        ...
        response_body['results'] = users
        return response_body, 200
    if request.method == 'POST':
        data = request.json
        users.append(data)
        response_body['message'] = ''
        response_body['results'] = users[-1]
        return response_body, 200
...
def handle_user(id_user):
    response_body = {}
    if id_users >= len(users):
        response_body['message'] = 'El usuario no existe'
        response_body['results'] = {}
        return response_body, 404
    if request.method == 'GET':
        response_body['message'] = 'Mensaje dese el GET'
        response_body['results'] = id_user
        return response_body, 200
    if request.method == 'PUT':
        response_body['message'] = 'Mensaje dese el PUT'
        response_body['results'] = id_user
        return response_body, 200
    if request.method == 'DELETE':
        response_body['message'] = 'Mensaje dese el DELETE'
        response_body['results'] = id_user        
        return response_body, 200
